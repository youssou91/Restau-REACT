                ...
                <SplideSlide key={item.id}>
                  <div className='rounded-3xl relative'>
                    <div className='absolute w-full h-full bg-black/50 rounded-3xl text-white'>
                      ...
                      {/* <p className='bg-orange-700 h-16 w-16 rounded-full mt-2 text-white py-2 px-2 border-8 font-bold'>{item.price}</p> */}
                      <button className='border-dotted border-white text-white mx-2 absolute bottom-4'>Add to Cart</button>
                    </div>
                    <img className='h-[200px] w-full object-cover rounded-3xl cursor-pointer hover:slace-105 ease-out duration-300'
                    src={item.img}
                    alt={item.title}/>
                  </div>
                </SplideSlide>
